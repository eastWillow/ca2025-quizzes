...
int main(void)
{
    uint32_t test_values[] = {0x00000000, 0x00000001, 0x0000000F, 0x00000010,
                              0x00000100, 0x00001000, 0x00010000, 0x00020000,
                              0x00040000, 0x00080000, 0x000F7FF0, 0x000FFFFF};

    uint8_t expect_values[] = {0x00, 0x01, 0x0F, 0x10, 0x41, 0x80,
                               0xC0, 0xD0, 0xE0, 0xF0, 0xFF, 0xFF};

    size_t n = sizeof(test_values) / sizeof(test_values[0]);

    for (size_t i = 0; i < n; i++) {
        uf8 encoded = uf8_encode(test_values[i]);
        printf("value=0x%08X -> uf8=0x%02X (expect 0x%02X)\n", test_values[i],
               encoded, expect_values[i]);
        assert(encoded == expect_values[i]);
    }
